...
-- age_groups / event_eligibility / status columns), run the migration
-- block at the bottom instead of the CREATE TABLE statements.
-- ============================================================
...
-- no need to retype the name/team/age group for every result.
...
create table if not exists public.entries (
  ...
  event_key       text not null,               -- '100m' | 'ljRun' | 'airrifle' | ...
  age_group       text not null,
  gender          text not null,               -- 'м' | 'ж'
  ...
  result_seconds  numeric,                     -- normalized seconds (track) or meters (field)
